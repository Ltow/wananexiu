...
import com.bossed.waej.ui.ExceptionHandlerActivity
...
class CrashHandler private constructor(context: Context) : Thread.UncaughtExceptionHandler {
    ...
    private fun handleException(e: Throwable) {
        Thread {
            Looper.prepare()
            Toast.makeText(mContext, "发生异常，正在退出", Toast.LENGTH_SHORT).show()
            Looper.loop()
        }.start()
        // 跳转到日志显示界面
        val intent = Intent(mContext, ExceptionHandlerActivity::class.java)
